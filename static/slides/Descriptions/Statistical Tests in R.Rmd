---
title: "A Very Incomplete Survey of Fundamental Statistical Tests in R"
subtitle: "EDP 613"
header-includes:
...
```{r echo = FALSE}
some_ed_data <- read_csv(here::here("static", "handouts", "some_ed_data.csv"))
```
...
```{r echo = FALSE}
some_exercise_data <- read_csv(here::here("static", "handouts", "some_exercise_data.csv"))
```
...
```{r echo = FALSE}
some_survey_data <- read_csv(here::here("static", "handouts", "some_survey_data.csv"))
```
...
\newpage
# A Final Rant About Open Source and Something Fun and Useless
...
On a side note, if you are a fan of the show Rick & Morty, consider downloading the most pointless package `mortyr` to do pointless statistics on pointless data. More about the package [here](https://github.com/mikejohnpage/mortyr). 

Or maybe you prefer that an "AI" write its own pointless [script](https://medium.com/dsc-manipal/generating-rick-and-morty-episodes-2413b608cd5) of the show.

Why? Because data science is fun!
