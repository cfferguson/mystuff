...
starwars_by_species %>%
  filter(species %in% c("Human", "Twi'lek", "Mon Calamari", "Wookiee")) %>%
 ...
 ggplot(aes(x = "", 
  ...
  geom_label(aes(y = ypos, 
                 label = paste0(`species`, " (", round(prop, 0), "%)"), 
             color = "white", 
             size = 6)) +
  scale_fill_gradient(low = "#d9534f", 
                      high = "#428bca") +
...
```{r sw20, include = FALSE, echo=TRUE}
starwars_by_species %>%
  arrange(desc(`mean age`)) %>%
  mutate(prop = `mean age` / sum(starwars_by_species$`mean age`) * 100) %>%
  mutate(ypos = cumsum(prop)- 0.5*prop) %>%
ggplot(aes(x = "", 
           y = prop, 
           fill = `mean age`)) +
...
`r chunk_reveal(chunk_name = "sw20", color = c("white", "white", "white"), width = c(100, 100))`
